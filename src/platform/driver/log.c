...
static hal_com_t uart = 
{
  ...
  .com_irq_rx_cb      = NULL_PTR,
  .com_irq_tx_cb      = NULL_PTR,
  .data               = &_data
};
...
void log_printlen(uint8_t *buf, uint32_t len)
{
  for(int i=0 ; i<len; i++)
  {
    _data = *buf++;
